...
class LoginForm(Form):
    ...
    def validate(self):
        ...
        email = Admin.query.filter_by(email=self.email.data).first()
        if not email:
...
class AddAdminForm(Form):
    ...
    def validate(self):
        ...
        if not initial_validation:
            return False
        user = Admin.query.filter_by(name=self.name.data).first()
        if user:
            self.nickname.errors.append("Adminname already registered")
            return False
        auth = Admin.query.filter_by(email=self.email.data).first()
        if auth:
...
class EditAdminForm(Form):
    ...
    def validate(self):
        ...
        if not initial_validation:
            return False
        user = (Admin.query
                .filter(Admin.name == self.name.data)
        ...
        if user:
            ...
            return False
        auth = (Admin.query
                .filter(Admin.email == self.email.data)
                .filter(Admin.id != current_user.id)
                .first())
        if auth:
            self.email.errors.append("Email already registered")
